...
1. Send our fuzzing payloads directly to L2's (to the child partition VM) harness, which will send the payload  to the target hypervisor.  
2. Retreive code coverage out of L1 (the target hypervisor)  
...
2. Child Partition - The guest VM, which executes the harness part as before.  
...
2. Retreive code coverage out of L2 (The root partition, our target)  
3. Monitor crashes on L2 (Root partition).  

Now that we understand exactly what are the basic blocks of hypervisor fuzzing, it's time to describe how each part was implemented.

## Code Coverage (Intel PT MSR)
kAFL leverages Intel PT in order to retreive code coverage.  
...
In order to enable code coverage of a nested VM, modification to the VM-Entry and VM-Exit part of KVM is required, in addition to enabling / disabling the `IA32_RTIT_CTL` MSR only when we are handling the root partition VM.

### Code modification for VM-Entry and VM-Exit
The modification was added within the `vmx_vcpu_run` function of `arch/x86/kvm/vmx/vmx.c` for VM-Entry (enabling Intel PT):
```C
    [... SNIP ...]
...
As mentioned before, this modification is working for Hyper-V's root partition, but you can easily modify it to support any other hypervisor, by using this condition instead of `is_root_partition`:
```
...
The same modification was also implemented for VM-Exit (disabling Intel PT) within `vmx_handle_exit_irqoff` function of `arch/x86/kvm/vmx/vmx.c`
```C
    [... SNIP ...]
...
KVM actually handles the exit reason of a nested VM before it is handled by L1 hypervisor. The target hypervisor doesn't know this exit reason as it's implemented by kAFL modifications, therefore we will have to tell KVM not to reflect this exit reason to L1, and eventually it will be handled within KVM.  

This modification was added within the `nested_vmx_reflect_vmexit` function of `arch/x86/kvm/vmx/nested.c`:
```C
    [ ... SNIP ...]
...
### Hypercalls
kAFL adds a hypercall which enables the guest VM to communicate with KVM directly (e.g. synchronization between the harness and the fuzzer, indicating about a crash, etc.)  

A hypercall is causing to a VM-Exit with the `EXIT_REASON_VMCALL` exit reason.  
We need to make sure that this exit reason will be handled within KVM and won't be transferred to the L1 hypervisor. In order to do so, we will modify the following code (the `nested_vmx_reflect_vmexit` function of `arch/x86/kvm/vmx/nested.c`:)
```C
    [... SNIP ...]
...
In contrast to QEMU, KVM is actually fully aware of the nested VM memory mapping, so when it tries to convert a GVA to GPA it actually uses the `vcpu->arch.walk_mmu` struct which is fully aware of any VM memory mapping including nested ones (by leveraging Intel EPT mechanism):  
```C
...
KVM implements an IOCTL named `KVM_TRANSLATE` which translates GVA to GPA, which of course works with nested GVAs. All I had to do is modify QEMU-PT so it will translate GVAs by leveraging KVM, instead of doing it on its own (the example is from the `analyse_assembly` function of `qemu-6.0.0/pt/disassembler.c`:)    
```C
    [ ... SNIP ... ]
...
In order to solve this part, I implemented two KVM IOCTLs: `KVM_VMX_PT_WRITE_TO_GUEST` and `KVM_VMX_PT_READ_FROM_GUEST` which enabling QEMU to read/write memory from a nested VM by using KVM instead of trying to perform it on its own.
...
The implementation of the KVM-side functionality can be found within the `kvm_arch_vcpu_ioctl` function of `/arch/x86/kvm/x86.c`.
