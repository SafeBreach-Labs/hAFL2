...
echo "================================================="
echo "           hAFL2 auto-magic installer             "
echo "================================================="
...
system_deps()
{
	...
	echo "[*] Creating /root/crashes folder ..."
	sudo mkdir /root/crashes

	echo "[*] Creating /root/tmp_crashes folder ..."
	sudo mkdir /root/tmp_crashes
}
...
build_qemu()
{
	...
	echo "[*] Building Capstone 4..."
	pushd "capstone"
	bash make.sh
	echo "[*] Installing libcapstone4..."
	bash make.sh install
	popd
	echo "[*] Building QEMU 6.0.0..."
	echo "-------------------------------------------------"
	...
	echo
	echo    QEMU_KAFL_LOCATION = qemu-${QEMU_VERSION}/build/x86_64-softmmu/qemu-system-x86_64
	echo
...
build_linux()
{
	...
	yes ""|make oldconfig
	#make localmodconfig
	./scripts/config --set-str CONFIG_LOCALVERSION "-hAFL2" --set-val CONFIG_KVM_VMX_PT y
...
install_note()
{
	...
	echo "Once you are done, reboot the system so the new kernel will be loaded."
}
