...
def make_sha1(data, encoding='utf-8'):
...
class qemu:
    ...
    def __init__(self, qid, config, debug_mode=False, notifiers=True):
        ...
        # hAFL1: Enable Hyper-V related logic, Intel PT for code coverage and disable vmx.
        self.cmd += " -serial file:" + self.qemu_serial_log + \
                    " -enable-kvm" \
                    " -cpu host,hv_relaxed,hv_spinlocks=0x1fff,hv_vapic,hv_time,+intel-pt,-hypervisor,+vmx -usbdevice tablet" \
                    " -m " + str(config.argument_values['mem']) + \
                    " -nographic" \
                    " -chardev socket,server=on,wait=off,path=" + self.control_filename + \
                    ",id=kafl_interface" \
        ...
        self.cmd += ",reload_mode=off"

        # qemu snapshots only work in VM mode (disk+ram image)
        if self.config.argument_values['kernel'] or self.config.argument_values['bios'] and not self.config.argument_values['vm_dir']:
            self.cmd += ",disable_snapshot=on"
        ...
        # Using a VM snapshot with a custom bios
    ...
    def __dump_recv_res(self, res):
        ...
        if self.handshake_stage_1 and res == qemu_protocol.RELEASE:
            ...
            os._exit(0)
        if res == qemu_protocol.CRASH or res == qemu_protocol.KASAN or res == qemu_protocol.CRASH_DUMP:
    ...
    def __debug_recv_expect(self, cmd):
        ...
        while True:
            ...
            # We need to factor out the debug stuff and properly support required vs optional/intermediate control messages...
                ...
                break
            elif res == qemu_protocol.CRASH_DUMP:
                break
            elif res == qemu_protocol.CRASH:
                break
            elif res is None:
    ...
    def restart(self):
        log_qemu("QEMU Restart!!!", self.qemu_id)
        self.shutdown()
    ...
    def check_recv(self, timeout_detection=False):
        ...
        if result == qemu_protocol.CRASH:
            ...
            return 7
        elif result == qemu_protocol.CRASH_DUMP:
            return 8
    ...
    def debug_payload(self, apply_patches=False):
    ...
    def send_payload(self, apply_patches=False, timeout_detection=False, max_iterations=10):
        ...
        if value == 0:
            ...
            log_qemu("Crash Dump!", self.qemu_id)
            while value in [1, 8]:
                value = self.check_recv(timeout_detection=timeout_detection)
                if value == 1:
                    pid = self.process.pid
                    log_qemu("Crash Detected after Crash Dump!, PID: %d" % pid, self.qemu_id)
                    with open(r"/root/tmp_crashes/%d.log" % pid, "rb") as f:
                        crash_dump = f.read()
                        crash_dump_sha1 = make_sha1(crash_dump)
                    current_payload_sha1 = make_sha1(self.current_payload)
                    crash_dump_payload_folder = r"/root/crashes/%s" % crash_dump_sha1

                    # A. Crash already exists, only drop payload
                    if os.path.exists(r"/root/crashes/%s.log" % crash_dump_sha1):
                        # B. Crash dump payload folder already exists
                        if os.path.exists(crash_dump_payload_folder):
                            # C. Payload already exists and connected to the crash, abort this one to avoid dups.
                            if os.path.exists(os.path.join(crash_dump_payload_folder, current_payload_sha1)):
                                log_qemu("Payload already existing...", self.qemu_id)
                            else:
                                with open(os.path.join(crash_dump_payload_folder, current_payload_sha1), "wb") as f:
                                    f.write(self.current_payload)
                        # B. Create Crash Dump Payloads folder
                        else:
                            os.mkdir(crash_dump_payload_folder)
                            # C. If we just created the folder, there is no payloads, so drop the payload.
                            with open(os.path.join(crash_dump_payload_folder, current_payload_sha1), "wb") as f:
                                f.write(current_payload)
                    # A. Crash doesn't exist, drop it and create a payloads folder
                    else:
                        with open(r"/root/crashes/%s.log" % crash_dump_sha1, "wb") as f:
                            f.write(crash_dump)
                        os.mkdir(crash_dump_payload_folder)
                        with open(os.path.join(crash_dump_payload_folder, current_payload_sha1), "wb") as f:
                            f.write(self.current_payload)

                    os.remove(r"/root/tmp_crashes/%d.log" % pid)
